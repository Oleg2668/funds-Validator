...
public class Main {
    ...
    public static void main(String[] args) {
        balance =getBalance();// add getBalance() method
    ...
    private static void validateAmount(double balance, double withdrawal) {
        if (withdrawal > balance) {  // add variable "balance"
            try {
                throw new FundsException("Insufficient funds!");
            } catch (FundsException ex) {
                System.out.println(ex.getMessage());// fix typo .getMessage()
            }
        } else {
    ...
    private static double getBalance(double balance, double withdrawal) {
        return balance - withdrawal;// add variable withdraw
    }
}
